...
public class LoginActivity extends BaseActivity implements OnClickListener {
	...
	private NetHandler loginHandler = new NetHandler(true) {
		...
		public void onSuccess(String errMsg, Message msg) {
			/*
			 * LoginResp resp = null; if (null != msg.obj && msg.obj instanceof
			 * LoginResp) {
			 * SharedPreferencesUtil.saveSharedPreferences(mContext,
			 * GlobalConstant.REMEMBER_PASSWORD, rememberCheckBox.isChecked());
			 * 
			 * resp = (LoginResp) msg.obj; UserItem item = new UserItem();
			 * item.phone = telphone; item.password = password; item.parentSex =
			 * resp.parentSex; item.userId = resp.userId; item.children =
			 * resp.children; LoginUtil.saveLoginInfo(mContext, item); Gson gson
			 * = new GsonBuilder().disableHtmlEscaping().create(); String
			 * cookieStr = gson.toJson(WXApplication.getInstance().cookieList);
			 * SharedPreferencesUtil.saveSharedPreferences(mContext,
			 * SharedPreferencesUtil.SP_KEY_LOGIN_COOKIE, cookieStr);
			 * finishActivity(item);
			 */
			LogX.getInstance().d("login", "成功！！！！");
	...
	public void initViews() {
		...
		telphoneEditText.setText("15251859531");
	...
	private void initData() {
		telphoneEditText.setText(SharedPreferencesUtil.getSharedPreferences(
				mContext, GlobalConstant.UserInfoPreference.TELPHONE, ""));
		passwordEditText.setText(SharedPreferencesUtil.getSharedPreferences(
				mContext, GlobalConstant.UserInfoPreference.PASSWORD, ""));
	...
	public void onClick(View v) {
		...
		case R.id.login_submit:
			...
			if (StringUtil.isEmpty(telphone)) {
				...
				// Map<String, String> reqMap = HttpRequestUtil
				// .getCommonParams("");
				...
				// 调用登录接口
				String url = HttpAddressProperties.LOGIN_URL + telphone + "|"
						+ password;
				ProgressUtil.showProgressDialog(this,
						getString(R.string.ysh_submit_login));
				HttpRequestUtil.sendHttpPostCommonRequest(this, url, null,
						LoginResp.class, loginHandler);
