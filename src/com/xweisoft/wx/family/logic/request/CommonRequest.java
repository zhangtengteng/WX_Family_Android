	...
	private String createRequestPara()
    {
        ...
        if (flag == HttpRequestUtil.REQUEST_JSON)
        {
            if (null != reqDataMap && reqDataMap.size() > 0)
            {
                ...
                LogX.getInstance().i(TAG, "post Data json: " + params);
				...
				if (((CommonResp) object).getCode().equals("200")) {
					if (null != handler) {
						handler.sendMessage(handler.obtainMessage(
								CommontSuccessCodes.SUCCESS, json));
					}
				}
			} else {
				if (null != handler) {
					handler.sendMessage(handler.obtainMessage(
							CommontSuccessCodes.SUCCESS, object));
				}
			}
		...
		} catch (Exception e) // 异常处理
		{
			if (null != handler) {
				handler.sendMessage(handler
						.obtainMessage(CommontErrorCodes.OTHER_ERROR));
			}
		}
	}
